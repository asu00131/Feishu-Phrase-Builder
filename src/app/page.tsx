...
export default function DashboardPage() {
  ...
  // Enhanced Pitch Items with auxiliary info
  const pitchItemsWithInfo = React.useMemo(() => {
    if (!selectedScene || !primaryFilterKey || !secondaryFilterKey) return []
    return data
      .filter(item => String(item[primaryFilterKey]) === selectedScene)
      .map(item => {
        const id = String(item[secondaryFilterKey])
        const timeInfo = getField(item, ['时间段', '时间段(分钟)', '时长'])
        return {
          id,
          display: timeInfo ? `${id} (${timeInfo})` : id,
          original: item
        }
      })
      .filter(item => item.id !== "undefined")
  }, [data, selectedScene, primaryFilterKey, secondaryFilterKey, getField])
  ...
  const handlePitchItemClick = (item: string) => {
    ...
    // Precise scroll to top of the card with offset for sticky header
    setTimeout(() => {
      scriptAreaRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })
    }, 50)
  }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F8FAFC]">
      <header className="bg-white border-b px-4 md:px-8 py-3 flex items-center justify-between sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1">
            <TabsList className="bg-slate-100/50 p-1 h-9 md:h-10">
              <TabsTrigger value="preview" onClick={scrollToTop} className="px-3 md:px-6 font-semibold text-xs md:text-sm">直播预览</TabsTrigger>
              <TabsTrigger value="config" onClick={scrollToTop} className="px-3 md:px-6 font-semibold text-xs md:text-sm">底层数据</TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
        <div className="flex items-center gap-2 ml-2">
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogTrigger asChild>
              <Button variant="outline" size="sm" className="rounded-full shadow-sm h-8 md:h-9 text-xs md:text-sm">
                <Settings className="h-4 w-4 md:mr-2" /> <span className="hidden xs:inline">数据源设置</span>
              </Button>
            </DialogTrigger>
            <DialogContent className="max-w-[95vw] md:max-w-lg rounded-2xl md:rounded-lg">
      ...
      <main className="flex-1 p-3 md:p-8">
        {activeTab === "preview" ? (
          <div className="grid grid-cols-12 gap-4 md:gap-6 max-w-[1700px] mx-auto">
            ...
            <div className="col-span-12 lg:col-span-7 space-y-6 md:space-y-10">
              ...
              <section className="space-y-3 md:space-y-4">
                <div className="flex items-center justify-between px-2">
                  <div className="flex items-center gap-2 md:gap-3">
                    <div className="bg-blue-100 p-1.5 md:p-2 rounded-xl"><Layers className="h-4 w-4 md:h-5 md:w-5 text-blue-600" /></div>
                    <h2 className="text-lg md:text-xl font-bold text-slate-800">策略流程话术</h2>
                  </div>
                  <Button variant="ghost" size="sm" onClick={() => loadData()} className="text-primary text-xs md:text-sm"><RotateCcw className="h-3 w-3 md:h-4 md:w-4 mr-1.5" /> 随机重置</Button>
                </div>
                {loading ? <div className="h-32 bg-white rounded-3xl animate-pulse" /> : 
                  <div className="space-y-4 relative">
                    <div className="absolute left-5 md:left-6 top-8 bottom-8 w-0.5 bg-slate-100" />
                    {Object.keys(strategySegments).map((module, i) => {
                      const idx = segmentIndices[module] || 0
                      const item = strategySegments[module][idx]
                      return (
                        <div key={module} className="relative pl-12 md:pl-14">
                          <div className="absolute left-0 top-0 w-10 h-10 md:w-12 md:h-12 bg-white border-2 border-slate-50 rounded-xl md:rounded-2xl flex items-center justify-center shadow-sm z-10 text-primary font-bold text-sm md:text-base">{i+1}</div>
                          <div className="bg-white rounded-2xl md:rounded-[2rem] p-4 md:p-6 shadow-sm border border-slate-100">
                            <div className="flex items-center justify-between mb-3">
                              <span className="font-bold text-slate-800 text-sm md:text-base">{module} <Badge variant="secondary" className="ml-2 font-normal text-[10px] md:text-xs">v{idx+1}</Badge></span>
                              <Button variant="outline" size="sm" onClick={() => handleShuffleProcess(module)} className="h-7 md:h-8 rounded-full text-[10px] md:text-xs">换个版本</Button>
                            </div>
                            <div className="bg-slate-50 rounded-xl md:rounded-2xl p-3 md:p-4 text-slate-700 text-xs md:text-sm leading-relaxed whitespace-pre-wrap">{getField(item, ['长段话术', '内容', '文本']) || "暂无内容"}</div>
                          </div>
                        </div>
                      )
                    })}
              ...
              {/* QA Section */}
              <section className="space-y-3 md:space-y-4 pt-4 border-t border-slate-200">
                <div className="flex items-center gap-2 md:gap-3 px-2">
                  <div className="bg-amber-100 p-1.5 md:p-2 rounded-xl"><MessageSquare className="h-4 w-4 md:h-5 md:w-5 text-amber-600" /></div>
                  <h2 className="text-lg md:text-xl font-bold text-slate-800">常见提问答疑</h2>
                </div>

                <div className="bg-white rounded-2xl md:rounded-[2rem] p-4 md:p-6 shadow-sm border border-slate-100 space-y-4 md:space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <Select value={selectedQACategory} onValueChange={setSelectedQACategory}>
                        <SelectTrigger className="rounded-xl h-9 md:h-10 font-bold border-slate-200"><SelectValue placeholder="选择类型" /></SelectTrigger>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1.5 md:gap-2 max-h-[160px] overflow-y-auto p-1 scrollbar-hide">
                      {qaItems.map(item => {
                        ...
                        return (
                          <Button
                            ...
                            className={cn(
                              "rounded-full px-2 md:px-3 h-7 md:h-8 text-[10px] md:text-xs font-semibold transition-all border-slate-200",
                  ...
                  <div ref={qaDetailRef} className="bg-slate-50 rounded-xl md:rounded-[1.5rem] p-3 md:p-4 border border-slate-100 space-y-3">
                    {activeQAScript ? (
                      <div className="space-y-3">
                        <div className="flex gap-2 md:gap-3 items-start">
                          <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-amber-500 flex items-center justify-center shrink-0 text-white font-bold text-[10px] md:text-xs shadow-sm">Q</div>
                          <div className="bg-amber-100/50 p-2.5 md:p-3 rounded-xl md:rounded-2xl rounded-tl-none border border-amber-200/50 flex-1">
                            <p className="text-amber-900 text-[11px] md:text-sm font-bold leading-snug">{String(activeQAScript[qaQuestionKey || ""] || "未知提问")}</p>
                          </div>
                        </div>
                        <div className="flex gap-2 md:gap-3 items-start">
                          <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-emerald-500 flex items-center justify-center shrink-0 text-white font-bold text-[10px] md:text-xs shadow-sm">A</div>
                          <div className="bg-white p-2.5 md:p-3 rounded-xl md:rounded-2xl rounded-tl-none border border-slate-100 flex-1 shadow-sm">
                            <p className="text-slate-700 text-xs md:text-base font-semibold leading-relaxed whitespace-pre-wrap">{String(activeQAScript[qaAnswerKey || ""] || "暂无对策")}</p>
                            {activeQAScript["政策口径"] && (
                              <div className="mt-2 pt-2 border-t border-dashed border-slate-100 text-[10px] md:text-[11px] text-emerald-600 font-bold flex items-center gap-1">
                                <Mic2 className="w-2.5 h-2.5 md:w-3 md:h-3" /> 政策：{activeQAScript["政策口径"]}
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="py-6 md:py-8 text-center text-slate-300 text-xs md:text-sm">请在上方选择具体问题</div>
                    )}
                  </div>
                </div>
              </section>
            ...
            <div className="col-span-12 lg:col-span-5 space-y-4 md:space-y-6">
              <div className="bg-white rounded-2xl md:rounded-[2.5rem] p-4 md:p-6 shadow-sm border border-slate-100">
                <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-6">
                  <div className="bg-emerald-100 p-1.5 md:p-2 rounded-xl"><ShoppingBag className="h-4 w-4 md:h-5 md:w-5 text-emerald-600" /></div>
                  <h2 className="text-lg md:text-xl font-bold text-slate-800">核心话术筛选</h2>
                </div>
                ...
                <div className="space-y-4 md:space-y-6">
                  <div className="space-y-1.5">
                    ...
                    <Select value={selectedScene} onValueChange={setSelectedScene}>
                      <SelectTrigger className="w-full h-9 md:h-10 rounded-xl border-slate-200 font-bold text-xs md:text-sm"><SelectValue /></SelectTrigger>
                      <SelectContent className="rounded-xl">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1.5 md:gap-2 max-h-[250px] md:max-h-[300px] overflow-y-auto p-1 scrollbar-hide">
                      {pitchItemsWithInfo.map(item => (
                        <Button
                          key={item.id}
                          variant={selectedItem === item.id ? "default" : "outline"}
                          size="sm"
                          onClick={() => handlePitchItemClick(item.id)}
                          className={cn(
                            "rounded-full px-3 md:px-4 h-8 md:h-9 text-[10px] md:text-sm font-semibold transition-all border-slate-200",
                            selectedItem === item.id ? "bg-primary text-white shadow-md scale-105" : "bg-white text-slate-600"
                          )}
                        >
                          {item.display}
                        </Button>
              ...
              {/* Pitch Display Card */}
              <div ref={scriptAreaRef} className="bg-white rounded-2xl md:rounded-[2.5rem] p-5 md:p-8 shadow-xl border border-slate-100 min-h-[300px] md:min-h-[400px] scroll-mt-20">
                <div className="flex flex-wrap items-center gap-2 mb-4 md:mb-6">
                  <Badge className="bg-emerald-500 text-white border-none px-3 md:px-4 py-0.5 md:py-1 rounded-full font-bold text-[10px] md:text-xs">核心讲解话术</Badge>
                  {selectedItem && <Badge variant="outline" className="border-emerald-200 text-emerald-600 px-2 md:px-3 py-0.5 md:py-1 rounded-full text-[10px] md:text-xs truncate max-w-[150px] md:max-w-none">{selectedItem}</Badge>}
                </div>
                
                <div className="text-slate-700 leading-relaxed text-sm md:text-lg font-semibold whitespace-pre-wrap">
                  {activePitchScript ? (
                    <div className="space-y-4 md:space-y-6">
                      {(activePitchScript["短句话术"] || activePitchScript["时间段"]) && (
                        <div className="bg-slate-50 p-2 md:p-3 rounded-xl text-[10px] md:text-sm text-slate-500 border-l-4 border-primary/50">
                          {activePitchScript["短句话术"] || `推荐时段: ${activePitchScript["时间段"]}`}
                        </div>
                      )}
                      <div className="text-xs md:text-lg">{getField(activePitchScript, ['长段话术', '内容', '文本']) || "未检测到讲解话术字段"}</div>
                      {activePitchScript["政策口径"] && (
                        <div className="mt-4 md:mt-6 pt-3 md:pt-4 border-t border-dashed text-[10px] md:text-sm text-emerald-600 font-bold flex items-start gap-2">
                          <Mic2 className="h-3 w-3 md:h-4 md:w-4 mt-0.5 md:mt-1 shrink-0" />
                          <span>政策提示：{activePitchScript["政策口径"]}</span>
                        </div>
                      )}
                    </div>
                  ) : (
                    <div className="flex flex-col items-center justify-center py-12 md:py-20 text-slate-300">
                      <ShoppingBag className="h-10 w-10 md:h-12 md:w-12 mb-4 opacity-10" />
                      <p className="text-[10px] md:text-sm">请选择具体产品以查看详情</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="max-w-[1400px] mx-auto p-2 md:p-0">
             <div className="bg-white rounded-xl md:rounded-[2.5rem] p-4 md:p-8 shadow-sm border border-slate-100 overflow-x-auto">
                <DataTable data={data} />
             </div>
          </div>
        )}
